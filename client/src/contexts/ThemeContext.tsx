...
import { setTheme, setPrimaryColor } from '@/store/features/themeSlice';
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  const { theme, primaryColor } = useSelector((state: RootState) => state.theme);
  ...
  useEffect(() => {
    ...
    if (savedColor) dispatch(setPrimaryColor(savedColor));
  }, [dispatch]);
  ...
  useEffect(() => {
    ...
    const effectiveColor = isGradient ? 'blue' : primaryColor;
  ...
  const setPrimaryColorAction = (color: PrimaryColor) => {
  ...
  const value: ThemeContextType = {
    ...
    setPrimaryColor: setPrimaryColorAction
  };
